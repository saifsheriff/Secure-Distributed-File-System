...
public class FileServant extends UnicastRemoteObject implements FileServerI {
    ...
    // FIX 4 & BONUS: replace raw userStore/sessions with managed versions
   ...
   public FileServant(int nodeId, int port, String storagePath,
                   List<int[]> peerConfig, int delayTargetPort,
                   RMISSLClientSocketFactory csf,
                   RMISSLServerSocketFactory ssf) throws RemoteException {
    // FIX 3: export this object using SSL sockets instead of plain TCP
    super(port, csf, ssf);
        this.nodeId = nodeId;
        this.port = port;
    ...
    public String register(String username, String password) throws RemoteException {
        // FIX 4: delegates to AuthManager which hashes the password
        String result = authManager.register(username, password);
        System.out.println("[Node " + nodeId + "] Register '" + username + "': " + result);
        return result;
    }

    @Override
    public String login(String username, String password) throws RemoteException {
        // FIX 4: AuthManager checks hashed password and issues expiring token
        String result = authManager.login(username, password);
    ...
    private void requireAuth(String token) throws RemoteException {
        try {
            authManager.requireAuth(token); // FIX 4: checks expiry
    ...
    public boolean uploadFile(String sessionToken, FileSerializable f) throws RemoteException {
        ...
        // VULNERABILITY: VUL-1 - No class whitelist; any deserialized object accepted
        // VULNERABILITY: VUL-2 - RMI dynamic class loading not disabled
    ...
    public FileSerializable downloadFile(String sessionToken, String fileName) throws RemoteException {
        ...
        try {
            File f = new File(storagePath + fileName);
            if (!f.exists()) {
                System.out.println("[Node " + nodeId + "] File not found: " + fileName);
                return null;
            }
    ...
    private synchronized void connectPeersIfNeeded() {
        ...
        for (int[] cfg : peerConfig) {
            try {
               // FIX 3: connect to peer registries over SSL
        ...
        if (peersConnected) {
            ...
            // Copy pending txIds first (outside clockLock to avoid deadlock)
